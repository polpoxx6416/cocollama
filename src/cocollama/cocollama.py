...
class OllamaShell(cmd.Cmd):
    ...
    def __init__(self, model: str = "gemma3:1b", host: str = "localhost", port: str = "11434"):
        super().__init__()
        self.model = model
        self.host = host
        self.port = port
    ...
    def _stream_response(self, data: Dict[str, Any]) -> None:
        ...
        with requests.post(url=api_endpoint, headers=headers, json=data, stream=True) as response:
            response.raise_for_status()
            self.last_response = ""
        ...
        for cmd in commands:
            try:
                ...
                if result.returncode == 0:
                    results[cmd] = f"<{cmd_id}_output>_{result.stdout.strip()}_</{cmd_id}_output>"
                else:
                    results[cmd] = f"<{cmd_id}_error>_{result.stderr.strip()}_</{cmd_id}_error>"
            except subprocess.TimeoutExpired:
                results[cmd] = f"<{cmd_id}_error>_Command timed out after 30 seconds_</{cmd_id}_error>"
            except Exception as e:
        ...
        try:
            with requests.get(url=urljoin(self.base_url, "/api/tags")) as response:
                ...
                for model in result["models"]:
                    modified_date = datetime.fromisoformat(model.get("modified_at", "unknown date").replace('Z', '+00:00')).date() if model.get("modified_at") else "unknown date"             
